...
show_menu() {
    ...
    echo_color "YELLOW" "3. Scripts Personalizados"
    echo_color "YELLOW" "4. Sair"
    echo
    echo_color "BLUE" "Escolha uma opção: "
}
    ...
    case $option in
        ...
        3)
            while true; do
                clear
                echo_color "BLUE" "=============================================="
                echo_color "BLUE" "     Scripts Personalizados"
                echo_color "BLUE" "=============================================="
                echo
                if list_scripts "meus_scripts" ""; then
                    read -r custom_option
                    if [[ "$custom_option" =~ ^[0-9]+$ ]]; then
                        num_scripts=$(ls meus_scripts/*.sh 2>/dev/null | wc -l)
                        if [ "$custom_option" -eq "$((num_scripts + 1))" ]; then
                            break
                        elif [ "$custom_option" -le "$num_scripts" ]; then
                            script_path=$(ls meus_scripts/*.sh | sed -n "${custom_option}p")
                            execute_script "$script_path"
                        else
                            echo_color "RED" "Opção inválida!"
                            read -p "Pressione ENTER para continuar..."
